...
<div class="row">
    <div class="col-sm-12">
        <div class="page-title-box">
            ...
            <h4 class="page-title">Master Karyawan Kontrak</h4>
        </div>
    </div>
</div>
    ...
    <div class="col-md-12">
        <div class="panel panel-color panel-teal">
            <div class="panel-heading">
                <h3 class="panel-title">Karyawan Kontrak Aktif</h3>
            </div>
            <div class="panel-body">
                <div class="row">
                    <div class="col-md-12">
                        <form id="form-filter-master-karyawan" role="form" action="<?=  base_url('MasterKaryawan/setFilterMasterKaryawan')?>" method="POST">
                            <div class="col-md-2">
                            ...
                            <div class="col-md-4 text-right">
                                <button id="btnMlUpdate" type="button" class="btn btn-sm btn-info"><i class="fa fa-table m-r-5"></i> Multiple Update</button>
                                <div class="btn-group">
                    ...
                    <div class="col-md-12">
                        <form id="form-filter-export-excel" role="form" method="POST">
                            <table id="tbl-mater-karyawan" class="table table-hover table-colored table-teal table-striped table-nowrap">
                                <thead>
                                    <tr>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td><?= ($r->TglKontrak != NULL ? date('F d, Y', strtotime($r->TglKontrak)) : '')?></td>
                                        <td><?= ($r->TglHabisKontrak != NULL ? date('F d, Y', strtotime($r->TglHabisKontrak)) : '')?></td>
                                        <td><?= $r->MASAKONTRAK?></td>
                                        <td><?= $r->KontrakKe?></td>
                                        ...
                                        <td><?= $r->jatah_cuti?></td>
                                        <td><?= ($r->TglKontrak!= '' && $r->TglHabisKontrak!= '' && $r->MASAKONTRAK!= '' && $r->KontrakKe!= '' ? '<label class="label label-success">Fixed</label>': '<label class="label label-danger">Not Fix</label>')?></td>
                                        <td>
                                            <a href="javascript:editMasterKary(<?= $r->NIK?>);" class="btn btn-xs btn-success waves-effect waves-light">
...
<div id="modal-edit-master-karyawan" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
    <div class="modal-dialog">
        <div class="modal-content">
            <form id="form-edit-mst-karyawan" action="<?= base_url('MasterKaryawan/updateMasterKary')?>" method="POST" role="form" >
...
<!-- Edit Master Karyawan Multiple via Excel -->
<div id="modal-imp-master-karyawan" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
    <div class="modal-dialog">
        <div class="modal-content">
            <form id="form-imp-mst-karyawan" action="<?= base_url('MasterKaryawan/importToUpdateMasterKary')?>" method="POST" role="form" enctype="multipart/form-data">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
...
<!-- Edit Master Karyawan Multiple with Modal -->
<div id="modal-edit-multiple" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="full-width-modalLabel" aria-hidden="true" style="display: none;">
    <div class="modal-dialog modal-full">
        <div class="modal-content">
            <form id="form-edit-multiple" action="<?= base_url('MasterKaryawan/updateMultiMasterKaryawan')?>" method="POST" role="form" >
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
...
<script type="text/javascript">
    var urlGetEdit  = '<?= base_url()?>MasterKaryawan/getMstKary',
        urlExpExcel = '<?= base_url()?>MasterKaryawan/exportMasterKaryawan',
        urlMultiUp  = '<?= base_url()?>MasterKaryawan/getMasterForUpdateMulti',
        urlPaging   = ['<?= base_url($this->uri->segment(1).'/'.$this->uri->segment(2))?>', '<?= $this->uri->segment(4)?>', '<?= $this->uri->segment(3)?>'],
        urlBase     = '<?= base_url()?>MasterKaryawan/index';
</script>
<script type="text/javascript" src="<?= base_url()?>assets/app/karyawan/master/kontrak/index.js"></script>