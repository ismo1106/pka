...
jQuery(document).ready(function () {
    ...
    var table;
    $('#btn-cari-boro, #txtInputNama').click(function (){
        table = $('#tbl-select-boro').DataTable({
            "sScrollY": "410px",
            "sScrollX": "100%",
            "sScrollXInner": "100%",
        ...
        $('#modal-select-boro').modal('show');
    });
    
    var selectedRow = function (){
        $('#tbl-select-boro tbody').on('click', 'tr', function () {
            ...
            $.post(urlGetData, {
                ...
                $('#txtInputNoFix').val(getRes.FixNo);
            ...
            $('#modal-select-boro').modal('hide');
        });
    };
    selectedRow();
    $('#modal-select-boro').on('hidden.bs.modal', function () {
        $('#modal-content-boro').html('<table id="tbl-select-boro" class="table table-striped table-hover table-nowrap">\n\
                                <thead>\n\
                                    <tr>\n\
                                        <th>No. Urut</th>\n\
                                        <th>NIK</th>\n\
    ...
    $('#btnSignature').click(function (){
        ...
        window.open(urlSignature+'?filename=hb'+fileName, '_blank');
    });
    
    //##== Button Submit =======================================================
    $('#btnSubmit').click(function (){
        $('#form-input-medical-boro').attr('action', urlSubmitMedic);
        if($('#form-input-medical-boro').parsley().validate()){
            var fileName = $('#txtInputNIK').val();
            $.post(urlCheckSign, {
                txtFileName : 'hb'+fileName
            }, function(data){
                if(data == true){
                    $('#form-input-medical-boro').submit();
                }else{
                    swal('Warning!','Silahkan tanda tangan terlebih dahulu!','warning');
                    $('#form-input-medical-boro').attr('action', '');
                }
            });
        }
    });
